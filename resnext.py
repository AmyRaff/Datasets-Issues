...
def train_model(model, epochs):
    ...
    for epoch in range(EPOCHS):
        ...
        if not os.path.exists(f"models/resnext"):
            os.makedirs(f"models/resnext")
        # Track best performance, and save the model's state
        if avg_vloss < best_vloss:
            best_vloss = avg_vloss
            model_path = 'models/resnext/model_{}'.format(epoch_number)
...
model = resnext101_32x8d(num_classes=6)
model.to('cuda')
