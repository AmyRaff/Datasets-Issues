...
def train_model(model, epochs):
    ...
    for epoch in range(EPOCHS):
        ...
        if not os.path.exists(f"models/convnext_small"):
            os.makedirs(f"models/convnext_small")
        # Track best performance, and save the model's state
        if avg_vloss < best_vloss:
            best_vloss = avg_vloss
            model_path = 'models/convnext_small/model_{}'.format(epoch_number)
...
model = convnext_small(num_classes=6)
model.to('cuda')
