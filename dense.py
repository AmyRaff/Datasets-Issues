...
def train_model(model, epochs):
    ...
    for epoch in range(EPOCHS):
        ...
        if not os.path.exists(f"models/densenet"):
            os.makedirs(f"models/densenet")
        # Track best performance, and save the model's state
        if avg_vloss < best_vloss:
            best_vloss = avg_vloss
            model_path = 'models/densenet/model_{}'.format(epoch_number)
...
# Class labels
#classes = ('MIMIC', 'CXR14', '', 'Effusion', 'Pneumonia', 'Pneumothorax')

# Report split sizes
print('Training set has {} instances'.format(len(training_set)))
print('Validation set has {} instances'.format(len(validation_set)))   

model = densenet161(num_classes=3)
model.to('cuda')
