...
def train_model(model, epochs):
    ...
    for epoch in range(EPOCHS):
        ...
        if not os.path.exists(f"models/efficient"):
            os.makedirs(f"models/efficient")
        # Track best performance, and save the model's state
        if avg_vloss < best_vloss:
            best_vloss = avg_vloss
            model_path = 'models/efficient/model_{}'.format(epoch_number)
...
model = efficientnet_v2_s(num_classes=6)
model.to('cuda')
